...
namespace NetCardID::utils::errors {
    ...
    //Extraction error
    ExtractionError::ExtractionError(const std::pair<drogon::HttpStatusCode, std::string_view>& message, const std::string_view& field) {
        message_ = std::string(message.second) + " " + std::string(field);
        code_ = message.first;
    }

    std::string ExtractionError::error() const {
        Json::Value jsonError;
        jsonError["Extraction-error"] = message_;
        return jsonError.toStyledString();
    }

    drogon::HttpStatusCode ExtractionError::code() const {
        return code_;
    }
}